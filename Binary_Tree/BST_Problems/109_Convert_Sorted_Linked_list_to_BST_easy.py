...
#以上这种方法的time complexity是 O（NlogN）具体的推到见下面link里第一种方法的complexity analysis
...
#对于每一种有特定数量的node或者element的list或者linked list，它们convert出来的BST的结构都是完全一样的

#也就是说我只要知道了我面对的这个linked list有几个node，我就可以算出来convert出来的BST应该是什么结构
#有几层，那个treenode有left child，有right child，哪个没child等等
...
#那么我们现在直到了我们convert出来的BST哪里放node哪里不放，现在要解决的问题就是，把这些sorted的value按
#什么order到每一个该放treenode的position里。答案是inorder，因为inorder traversal一个BST得到的是从小
