...
# 对于sorted arrays先取最左和最右两个极端值，如果两者之和小于target，那最小的和最大的相加都不够target => 最小的和其他小于max的elements相加就更不够了，因此排除min在wanted pair中的可能=>left index+1 同理两数之和大于target的话就把 right index-1
